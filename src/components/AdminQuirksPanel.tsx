...
import { isWorldCupEvent } from "@/lib/world-cup-constants";
import {
  QUIRK_CATEGORIES,
...
export function AdminQuirksPanel({
  eventSlug,
  eventTitle,
}: {
  eventSlug?: string;
  ...
  const apiBase = eventSlug
    ? `/api/events/${eventSlug}/admin/quirks`
    : "/api/admin/quirks";
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    setLoading(false);
  }, [apiBase, statusFilter, categoryFilter, searchDebounced]);
  ...
  async function exportCsv() {
    ...
    try {
      const query = buildQuery(statusFilter, categoryFilter, searchDebounced);
      const res = await fetch(`${apiBase}/export?${query}`);
        ...
        <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="max-w-2xl text-sm text-hawk-300">
              {eventSlug
                ? isWorldCupEvent(eventSlug)
                  ? "Live kit quirk reports from guests on the World Cup LAN site."
                  : `Reports submitted in the ${eventTitle ?? eventSlug} tournament space.`
                : "World Cup / LAN demo reports only. Open an event for tournament-specific quirks."}{" "}
              Auto-tagged by problem type (e.g.{" "}
              <Link
                href={
                  eventSlug
                    ? `/events/${eventSlug}/quirks?category=network`
                    : "/admin/quirks?category=network"
                }
